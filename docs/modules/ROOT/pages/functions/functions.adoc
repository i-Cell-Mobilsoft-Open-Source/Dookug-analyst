...
xref:additional.adoc#uploadingTemplatesToTheDatabase[The guide for uploading templates to the database can be found here.] If the template content changes during runtime, the /evict endpoint must be called to refresh the cache. This is not necessary when inserting a new template. 
...
xref:additional.adoc#electronicSignature[More information about the types of signatures can be found here.]
