...
The module is capable of generating documents in multiple ways. After generation, the module can also return the generated documents in gzip-compressed format.
...
Generated documents saved in the module database can be downloaded individually by their identifier. The response to the request returns the document in octet-stream format, as well as the file name. Upon request, the document can also be delivered in gzip-compressed format.
